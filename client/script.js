...
function changeHelloColor() {
    sayHelloButton.classList.add('buttonStyle')
}
...
// CODE HERE
function changeHelloColorBack() {
    sayHelloButton.classList.add('buttonDefault')
}
sayHelloButton.addEventListener('mouseout', changeHelloColorBack)
...
const repeatMyParam = () => {
    //YOUR CODE HERE
    axios.get(`${baseURL}/repeat/{Hello world}`)
        .then(res => {
            console.log(res.data)
...
function query() {
    const name = 'Griffin'
    axios.get(`${baseURL}/query-test?name=${name}`)
        .then(res => {
            console.log(res.data)
        })
